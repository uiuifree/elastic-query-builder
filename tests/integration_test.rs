...
pub fn case01() {
    ...
    assert_eq!(
        r#"{"query":{"match_all":{}},"script":{"lang":"painless","source":"ctx._source.count++"}}"#,
...
pub fn case02() {
    ...
    let agg = Aggregation::nested("name")
        .set_path("city")
        .append_aggregation(filter);
    builder.set_aggregation(vec![agg]);
    assert_eq!(
        builder.build().to_string(),
